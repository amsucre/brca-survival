...
### Internal Imports
from MGCT_MCAT.datasets.dataset_survival import Generic_MIL_Survival_Dataset
from MGCT_MCAT.utils.file_utils import save_pkl, load_pkl
from MGCT_MCAT.utils.core_utils import train
from MGCT_MCAT.utils.utils import get_custom_exp_code
...
def main(args):
	...
	for i in folds:
		...
		if args.split_dir.split('/')[3] == 'tcga_blca':
			...
			if args.apply_sigfeats:
				args.omic_sizes = [91, 353, 553, 480, 1566, 480]      # Originally this had 1565, but gives an error so manually modify to 1566
			...
			if args.apply_sigfeats:
				args.omic_sizes = [93, 329, 528, 457, 222, 447]  # Originally this had 1565, but gives an error so manually modify to 1566
				args.omic_input_dim = 3189
			else:
				args.omic_sizes = [91, 353, 553, 480, 1566, 480]
				args.omic_input_dim = 20970
...
args.split_dir = str(TrainConfig.splits_dir_survival)
