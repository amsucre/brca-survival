...
def collate_MIL_survival(batch):
    ...
    label = torch.LongTensor([int(item[2].item()) for item in batch])
...
def get_split_loader(split_dataset, training = False, testing = False, weighted = False, mode='coattn', batch_size=1):
    ...
    train_loader = loader
    num_batches = len(train_loader) if hasattr(train_loader, '__len__') else sum(1 for _ in train_loader)
    print(f"Number of batches in train_loader: {num_batches}")

    return loader


def get_evaluation_loader(split_dataset, batch_size=1):
    """
        return either the validation loader or training loader
    """
    ...
    num_batches = len(loader) if hasattr(loader, '__len__') else sum(1 for _ in loader)
    print(f"Number of batches in loader: {num_batches}")
...
def make_weights_for_balanced_classes_split(dataset, epsilon=1e-6):
    N = float(len(dataset))
    for c in range(len(dataset.slide_cls_ids)):
        print(f"Class {c}: {len(dataset.slide_cls_ids[c])} samples")
    weight_per_class = [N / (len(dataset.slide_cls_ids[c]) + epsilon) for c in range(len(dataset.slide_cls_ids))]                                                                                                                                            
