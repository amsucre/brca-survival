...
from PORPOISE.utils.utils import generate_split, nth
...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self,
        ...
        #slide_data['survival_months'] = np.floor(slide_data['survival_months']).astype(int)
        ...
        # metadata = ['disc_label', 'slide_id', 'case_id', 'label', 'site', 'is_female', 'oncotree_code', 'age', 'survival_months',  'censorship']
        # print(slide_data.columns[:11])
        # self.metadata = slide_data.columns[:10]
        metadata = ['disc_label', 'Unnamed: 0', 'case_id', 'label', 'slide_id', 'age', 'site', 'survival_months', 'censorship', 'is_female', 'oncotree_code']
        ...
        for col in slide_data.drop(self.metadata, axis=1).columns:
            ...
            if not pd.Series(col).str.contains('|_CLIN|_SNP|_EXP|_CNV|_MIR')[0]:
                print(col)
        #pdb.set_trace()

        assert self.metadata.sort_values().equals(pd.Index(sorted(metadata)))
        self.mode = mode
        self.cls_ids_prep()
    ...
    def return_all_splits(self, from_id: bool=True, csv_path: str=None):
        if from_id:
            raise NotImplementedError
        else:
            assert csv_path 
    ...
    def __getitem__(self, idx):
        ...
        if not self.use_h5:
            if self.data_dir:
                ...
                elif self.mode == 'tissues':
                    features = torch.tensor([[tumor.item(), stroma.item(), other.item()]])
                    return (features,  torch.zeros((1,1)), label, event_time, c)

                elif self.mode == 'omic':
                    genomic_features = torch.tensor(self.genomic_features.iloc[idx])
                    return (torch.zeros((1,1)), genomic_features.unsqueeze(dim=0), label, event_time, c)

                elif self.mode == 'pathomic' or self.mode == 'early':
                    path_features = []
                    for slide_id in slide_ids:
                        wsi_path = os.path.join(data_dir, 'pt_files', '{}.pt'.format(slide_id.rstrip('.svs')))
                        wsi_bag = torch.load(wsi_path)
...
class Personalized_MIL_Survival_Dataset(Generic_WSI_Survival_Dataset):
    def __init__(self, data_dir, mode: str = 'omic', **kwargs):
        super(Personalized_MIL_Survival_Dataset, self).__init__(**kwargs)
        self.data_dir = data_dir
        self.mode = mode
        self.use_h5 = False
    ...
    def __getitem__(self, idx):
        ...
        event_time = torch.Tensor([self.slide_data[self.label_col][idx]])
        c = torch.Tensor([self.slide_data['censorship'][idx]])
        tumor = torch.Tensor([self.slide_data['tumor'][idx]])
        stroma = torch.Tensor([self.slide_data['stroma'][idx]])
        other = torch.Tensor([self.slide_data['other'][idx]])
        slide_ids = self.patient_dict[case_id]
        ...
        if not self.use_h5:
            if self.data_dir:
                features = torch.tensor([[tumor.item(), stroma.item(), other.item()]])
                return (features, label, event_time, c)
            else:
                return slide_ids, label, event_time, c
