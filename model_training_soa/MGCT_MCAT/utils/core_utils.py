...
from MGCT.datasets.dataset_generic import save_splits
from MGCT.models.model_genomic import SNN, MLPOmics, MaskedOmics
from MGCT.models.mm_baselines import MIL_Sum_FC_surv, MIL_Attention_FC_surv, MIL_Cluster_FC_surv, MCAT_Surv, PorpoiseMMF
# from models.model_coattn import MCAT_Surv
from MGCT.models.MGCT import MGCT
from MGCT.models.new_MGCT import MGCT_Surv
from MGCT.utils.utils import *

from MGCT.utils.coattn_train_utils import *
from MGCT.utils.cluster_train_utils import *
...
def summary_survival(model, loader, n_classes):
    ...
    for batch_idx, (data_WSI, data_omic, label, event_time, c) in enumerate(loader):
        ...
        # modified, np.asscalar does not work anymore
...
def summary_survival_coattn(model, loader, n_classes):
    ...
    for batch_idx, (data_WSI, data_omic1, data_omic2, data_omic3, data_omic4, data_omic5, data_omic6, label, event_time, c) in enumerate(loader):
        ...
        #risk = np.asscalar(-torch.sum(survival, dim=1).cpu().numpy())    # modify as not used in updated version of torch and numpy
        #event_time = np.asscalar(event_time)
        #c = np.asscalar(c)
        risk = -torch.sum(survival, dim=1).cpu().numpy().item()
        event_time = event_time.item()
