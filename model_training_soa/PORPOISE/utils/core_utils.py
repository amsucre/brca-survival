...
from PORPOISE.datasets.dataset_generic import save_splits
from PORPOISE.models.model_genomic import SNN
from PORPOISE.models.model_set_mil import MIL_Sum_FC_surv, MIL_Attention_FC_surv, MIL_Cluster_FC_surv
from PORPOISE.models.model_coattn import MCAT_Surv
from PORPOISE.models.model_porpoise import PorpoiseMMF, PorpoiseAMIL, RegressionModel
from PORPOISE.models.model_early import EarlyMMF
from PORPOISE.utils.utils import *
from PORPOISE.utils.loss_func import NLLSurvLoss

from PORPOISE.utils.coattn_train_utils import *
from PORPOISE.utils.cluster_train_utils import *
...
def train(datasets: tuple, cur: int, args: Namespace):
    ...
    if args.model_type == 'porpoise_mmf':
    ...
    elif args.model_type == 'early_mmf':
        model_dict = {'omic_input_dim': args.omic_input_dim, 'fusion': args.fusion, 'n_classes': args.n_classes,
        'gate_path': args.gate_path, 'gate_omic': args.gate_omic, 'scale_dim1': args.scale_dim1, 'scale_dim2': args.scale_dim2,
        'skip': args.skip, 'dropinput': args.dropinput, 'path_input_dim': args.path_input_dim, 'use_mlp': args.use_mlp,
        }
        ...
        model_dict = {'n_classes': args.n_classes}
        model = RegressionModel(**model_dict)
    else:
        raise NotImplementedError
...
def train_alternative(datasets: tuple, cur: int, args: Namespace):
    """   
        train for a single fold
    """
    print('\nTraining Fold {}!'.format(cur))
    ...
    if args.model_type == 'porpoise_mmf':
        ...
        model_dict =  model_dict = {'omic_input_dim': args.omic_input_dim, 'fusion': args.fusion, 'n_classes': args.n_classes,
        'gate_path': args.gate_path, 'gate_omic': args.gate_omic, 'scale_dim1': args.scale_dim1, 'scale_dim2': args.scale_dim2,
        'skip': args.skip, 'dropinput': args.dropinput, 'path_input_dim': args.path_input_dim, 'use_mlp': args.use_mlp,
        }
        model = EarlyMMF(**model_dict)
...
#    torch.save(model.state_dict(), os.path.join(args.results_dir, "s_{}_checkpoint.pt".format(cur)))
    ...
    if args.mode == 'coattn':
        results_train_dict, train_cindex = summary_survival_coattn(model, train_loader, args.n_classes)
        results_val_dict, val_cindex = summary_survival_coattn(model, val_loader, args.n_classes)
    else:
        results_train_dict, train_cindex = summary_survival(model, train_loader, args.n_classes)
        results_val_dict, val_cindex = summary_survival(model, val_loader, args.n_classes)

    print('Val c-Index: {:.4f}'.format(val_cindex))
    writer.close()
    return results_train_dict, train_cindex, results_val_dict, val_cindex
...
def summary_survival(model, loader, n_classes):
    ...
    for batch_idx, (data_WSI, data_omic, y_disc, event_time, censor) in enumerate(loader):
        ...
        with torch.no_grad():
            h_original = model(x_path=data_WSI, x_omic=data_omic)
        ...
        all_event_times[batch_idx] = event_time
        patient_results.update({slide_id: {'slide_id': np.array(slide_id), 'model_output': h_original.cpu().numpy().tolist(), 
                                          'risk': risk, 'disc_label': y_disc.item(), 'survival': event_time, 'censorship': censor}})

    c_index = concordance_index_censored((1-all_censorships).astype(bool), all_event_times, all_risk_scores, tied_tol=1e-08)[0]
    return patient_results, c_index


def train_tissue_percentages(datasets: tuple, cur: int, args: Namespace):
    """
        train for a single fold
    """
    print('\nTraining Fold {}!'.format(cur))
    ...
    for epoch in range(args.max_epochs):
        if args.task_type == 'survival':
            train_loop_survival_tp(epoch, model, train_loader, optimizer, args.n_classes, writer, loss_fn, reg_fn,
                                args.lambda_reg, args.gc)
            stop = validate_survival_tp(cur, epoch, model, val_loader, args.n_classes, early_stopping, monitor_cindex,
                                     writer, loss_fn, reg_fn, args.lambda_reg, args.results_dir)

    torch.save(model.state_dict(), os.path.join(args.results_dir, "s_{}_checkpoint.pt".format(cur)))
    model.load_state_dict(torch.load(os.path.join(args.results_dir, "s_{}_checkpoint.pt".format(cur))))

    results_val_dict, val_cindex = summary_survival_tp(model, val_loader)
...
def train_loop_survival_tp(epoch, model, loader, optimizer, n_classes, writer=None, loss_fn=None, reg_fn=None,
                        lambda_reg=0., gc=16):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.train()
    train_loss_surv, train_loss = 0., 0.
    ...
    for batch_idx, (data_WSI, data_omic, y_disc, event_time, censor) in enumerate(loader):
        ...
        censor = censor.to(device)
        # pdb.set_trace()
        h = model(data_WSI)  # return hazards, S, Y_hat, A_raw, results_dict
        if not isinstance(h, tuple):
            loss = loss_fn(h=h, y=y_disc, t=event_time, c=censor)
            loss_value = loss.item()
        else:
        ...
        if y_disc.shape[0] == 1 and (batch_idx + 1) % 100 == 0:
            print('batch {}, loss: {:.4f}, label: {}, event_time: {:.4f}, risk: {:.4f}, bag_size: {}'.format(batch_idx,
                                                                                                             loss_value + loss_reg,
                                                                                                             y_disc.detach().cpu().item(),
                                                                                                             float(
                                                                                                                 event_time.detach().cpu().item()),
                                                                                                             float(
                                                                                                                 risk),
                                                                                                             data_WSI.size(
                                                                                                                 0)))
...
def validate_survival_tp(cur, epoch, model, loader, n_classes, early_stopping=None, monitor_cindex=None, writer=None,
...
def summary_survival_tp(model, loader):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    test_loss = 0.
    ...
    for batch_idx, (data_WSI, data_omic, y_disc, event_time, censor) in enumerate(loader):
        ...
        patient_results.update(
            {slide_id: {'slide_id': np.array(slide_id), 'model_output': h_original.cpu().numpy().tolist(),
                        'risk': risk, 'disc_label': y_disc.item(), 'survival': event_time, 'censorship': censor}})
