...
from models.model_utils import SNN_Block, Attn_Net_Gated, BilinearFusion, LRBilinearFusion, MLP_Block, Reg_Block, AFF, iAFF, Attn_Net
...
#                  use_ffn=True, use_trans=True, use_linear=True, attention='mha', use_gap=True):
#         super(MGCT_Layer, self).__init__()
#         self.fusion, self.use_gap = fusion, use_gap
...
#         #     self.beta_gap = Attn_Net_Gated(L=dim, D=dim, dropout=drop_rate, n_classes=1)
#         # else:
#         #     self.alpha_gap = Attn_Net(L=dim, D=dim, dropout=drop_rate, n_classes=1)
#         #     self.beta_gap = Attn_Net(L=dim, D=dim, dropout=drop_rate, n_classes=1)
...
#         elif fusion == 'lrbilinear':
#             self.gamma = LRBilinearFusion(dim1=dim, dim2=dim, scale_dim1=8, scale_dim2=8)
#         elif fusion == 'attn':
...
#             # print(omic_wsi_embed.shape, wsi_omic_embed.shape)
#             fused_embed = self.gamma(omic_wsi_embed * wsi_omic_embed)
#             print(fused_embed.shape)
#         else:
#             raise NotImplementedError
