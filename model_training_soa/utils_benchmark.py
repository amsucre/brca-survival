...
def summary_survival(model, loader, fold_num, model_type, features_path):
    device=torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.eval()
    test_loss = 0.
    ...
    if model_type=='porpoise_mmf' or model_type=='regression' or model_type=='porpoise_amil' or model_type=='snn' or model_type=='early_mmf':
        for batch_idx, (data_WSI, data_omic, y_disc, event_time, censor) in enumerate(loader):
            data_WSI, data_omic = data_WSI.to(device), data_omic.to(device)
            
            slide_id = slide_ids.iloc[batch_idx]

            # if model_type=='porpoise_amil':
            #     with torch.no_grad():
            #         features = model.get_slide_features(x_path=data_WSI)
            #         print('features amil shape', features.shape)
            #         print('path:', Path(features_path, slide_id.replace('.svs', '.pt')))
            #         torch.save(features, Path(features_path, slide_id.replace('.svs', '.pt')))
            ...
            patient_results.update({slide_id: {'slide_id': np.array(slide_id), 'model_output': h_original.cpu().numpy().tolist()[0],
                                                'hazards': hazards, 'survival': survival, 'risk': risk[0], 'disc_label': y_disc.item(), 
                                                'event_time': event_time, 'censorship': censor}})
    else:
        for batch_idx, (data_WSI, data_omic1, data_omic2, data_omic3, data_omic4, data_omic5, data_omic6, y_disc, event_time, censor) in enumerate(loader):
            ...
            y_disc = y_disc.type(torch.LongTensor).to(device)
            censor = censor.type(torch.FloatTensor).to(device)
            slide_id = slide_ids.iloc[batch_idx]

            with torch.no_grad():
                logits, hazards, survival, Y_hat, _  = model(x_path=data_WSI, x_omic1=data_omic1, x_omic2=data_omic2, x_omic3=data_omic3, x_omic4=data_omic4, x_omic5=data_omic5, x_omic6=data_omic6) # return hazards, S, Y_hat, A_raw, results_dict
            ...
            patient_results.update({slide_id: {'slide_id': np.array(slide_id), 'model_output': logits.cpu().numpy().tolist()[0],
                                                'hazards': hazards, 'survival': survival, 'risk': risk, 'disc_label': y_disc.item(), 
                                                'event_time': event_time, 'censorship': censor}})
