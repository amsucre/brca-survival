...
import torch
from CLAM.utils.constants import MODEL2CONSTANTS
from CLAM.utils.transform_utils import get_eval_transforms
...
def get_encoder(model_name, target_img_size=224, uni_checkpoint_path=None):
    print('loading model checkpoint')
    if model_name == 'resnet50_trunc':
        model = TimmCNNEncoder()
    elif model_name== 'convnext':
        print('WITH CONVNEXT')
        weights_model = 'random_convnext_base_TIGER_512_4classes_80epochs_dice_1times_mag1'
        device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        model, out_dir = load_model(TrainConfig, weights_model, 4, device)
        ...
        model.load_state_dict(torch.load(uni_checkpoint_path, map_location="cpu"), strict=True)
